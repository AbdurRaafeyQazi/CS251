...
string rot(const string& line, int amount) { // So these are overloading ?
  ...
  for(char ch : line){
    ...
    }else if(isspace(ch)){ // if this is a space then add it to the result or else don't
...
void rot(vector<string>& strings, int amount) {
  // this string contains multiple strings and you want this function to apply to each string.
  // one way to do this is to this is to use a enhanced for loop to interate over each element in the vecotr

  for(string& e : strings){
    // for each element; in the vec; modify the value of the element and store it back in the element.

    e = rot(e, amount); // put the element into the function for modification and then store it back into the element.
...
vector<string> splitBySpaces(const string& s) {
  // if you iterate 1 element at a time in a vector it will count each one as a seperate element 
  // so you need to store all of this in a string and then pushback this into the vector
...
string joinWithSpaces(const vector<string>& words) {
  ...
  for(size_t i = 0; i < words.size(); ++i){
    result += words[i]; // vector stores values not characters so at the i position will be a full value 
...
vector<char> decryptSubstCipher(const QuadgramScorer& scorer,
  ...
  // Generate an initial random key (A-Z mapping)
    vector<char> bestKey = genRandomSubstCipher();
    string bestDecryption = applySubstCipher(bestKey, ciphertext);
    double bestScore = scoreString(scorer, bestDecryption);

    // Perform simulated annealing or other optimization heuristic to improve the key
    int iterations = 1000; // Number of iterations to try different key permutations
    for (int i = 0; i < iterations; ++i) {
        // Copy the current best key and randomly swap two letters
        vector<char> newKey = bestKey;
        swap(newKey[rand() % 26], newKey[rand() % 26]); // Swap two random characters

        // Decrypt the text with the new key
        string newDecryption = applySubstCipher(newKey, ciphertext);

        // Score the new decryption
        double newScore = scoreString(scorer, newDecryption);

        // If the new decryption is better, update the best key and best score
    ...
    return bestKey; // Return the best key found
}
...
void decryptSubstCipherCommand(const QuadgramScorer& scorer) {
  string ciphertext;
    ...
    // Apply the best key to get the plaintext
